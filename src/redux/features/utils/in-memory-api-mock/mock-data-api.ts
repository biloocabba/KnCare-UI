...
import { mockAxiosReponse, businessUnitsMockResponse, countriesMockResponse, careMembersMockResponse, employeeMockResponse } from './mock-data';
...
export const saveCareMember = async (
  ...
  let employeeData: Employee = findEmployeeById(body.id);
  let careMemberResponse = {
    ...employeeData,
    ...body
  }

  let response = wrapIntoResponse(careMemberResponse);
...
const entitySearch = <T>(url: string, mockResponse: AxiosResponse<T[]>, filter: QueryFilterFunction<T>): AxiosResponse<any> => {
...
const filterEmployees = (queryParams: URLSearchParams, employeesData: Employee[]): Employee[] => {
  ...
  let result: Employee[] = employeesData.filter((employee) => {
    return matchFirstName(queryParams, employee) &&
...
export const filterCareMembers = (queryParams: URLSearchParams, careMembersData: CareMember[]): CareMember[] => {
  ...
  let result: CareMember[] = careMembersData.filter((careMember) => {
    ...
    return matchFirstName(queryParams, careMember) &&
